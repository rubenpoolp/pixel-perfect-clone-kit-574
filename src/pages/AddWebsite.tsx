...
const AddWebsite = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const {
      ...
      sanitizedUrl,
      error
    } = validateAndSanitizeUrl(websiteUrl);
    if (!isValid) {
      toast({
        title: "Invalid URL",
        description: error,
  ...
  return <div className="bg-white flex flex-col items-stretch py-[17px] min-h-screen">
      <Header />
      ...
      <main className="self-center flex w-full max-w-[800px] flex-col max-md:max-w-full px-8 py-16">
        ...
        <form onSubmit={handleSubmit} className="bg-[rgba(247,244,237,1)] shadow-[0px_20px_25px_-5px_rgba(0,0,0,0.1)] border flex flex-col items-stretch p-6 sm:p-8 rounded-2xl sm:rounded-[28px] border-[rgba(28,28,28,0.2)] border-solid">
          ...
          <Button type="submit" className="bg-[rgba(28,28,28,1)] text-[rgba(252,251,248,1)] hover:bg-[rgba(28,28,28,0.9)] font-medium py-3 px-6 rounded-lg transition-colors" disabled={!websiteUrl.trim() || !productType.trim()}>
            Start Analyzing
          </Button>
        </form>
        ...
        <div className="mt-8 text-center">
          <p className="text-[rgba(95,95,93,1)] text-sm">
            ...
            <button onClick={() => window.open('https://calendly.com/ruben-clutch/30min', '_blank')} className="text-[rgba(28,28,28,1)] font-medium ml-1 hover:underline">
              Request a demo
            </button>
          </p>
        </div>
